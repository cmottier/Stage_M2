...
# cellule d'aire 1 -> pas de cellarea
...
lambda_v <- as.im(exp(beta[1]+beta[2]*X+beta[3]*X2), square(dim))
...
resume <- function(out) {
  res <- NULL 
  res$avec <- MCMCsummary(out$avec, param=c("alpha", "beta"))
  res$sans <- MCMCsummary(out$sans, param=c("beta"))
  return(res)
}
