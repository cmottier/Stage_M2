...
# Exemple d'un cas défavorable : effort faible
# effort : b = plogis(-4+1*w)
# intensité : l = exp(-2+4*x1+1*x2)
...
alpha <- c(-4, 1) # effort : b = plogis(...+...*w)
beta <- c(-2, 4, 1) # intensité : l = exp(...+...*x)
...
for (i in 1:n) {
  ...
  dat <- simu(dat)
  out <- estim(dat)
  save(out, file = paste0("simu", i, "_scenario3.RData"))
  resultat[[i]] <- resume(out)
}

save(resultat, file = "simulation_scenario3_test.RData")
